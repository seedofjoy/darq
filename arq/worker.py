...
class Worker:
    ...
    def __init__(
        ...
        self.poll_delay_s = to_seconds_strict(poll_delay)
        self.queue_read_limit = queue_read_limit or max_jobs
        self._queue_read_offset = 0
        self.max_tries = max_tries
        self.health_check_interval = to_seconds_strict(health_check_interval)
    ...
    async def run_check(
        ...
        """
        Run :func:`arq.worker.Worker.async_run`, check for failed jobs
        and raise :class:`arq.worker.FailedJobs` if any jobs have failed.
